...
import { handleMemoryInject } from "../../../src/hooks/memoryInject.hook.ts";
import { parseMemoryInjectPayload } from "../../../src/hooks/payload.ts";
...
describe("UserPromptSubmit (memory-inject) hook", () => {
  ...
  test("fewer than 2 salient tokens: silent, no search, no log", async () => {
    const fixture = makeFixture();
    await seedIndexedWorkspace(fixture);
    // "are"/"you"/"the" are all stopwords (`domain/tokenize.ts`'s STOPWORDS);
