...
export default function App() {
  ...
  const { config: graphConfig, setConfig: setGraphConfig, reset: resetGraphConfig } = useGraphPhysics();

  const { typeFilter, tagFilter, featureFilter, filters: searchApiFilters } = searchFilters;
  ...
  const noteQuery = useNote(
    activeWs,
    activePath && !isWorklogTimeline ? activePath : "",
  );
  ...
  const graphQuery = useGraph(activeWs, activePath || null, depth, fullGraph, Boolean(activeWs));
  ...
  const openPath = useCallback(
    (path: string, _newTab = false): void => {
      setTabs((prev) => {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100vh", overflow: "hidden" }}>
            ...
            <div style={{ maxHeight: 360, overflow: "auto", padding: 8 }}>
              ...
              {!q.trim() && !searchFilters.hasFilters && (
                <div style={{ padding: "12px 14px", color: "var(--muted)", fontSize: 11 }}>
                  Tips: <code style={{ background: "var(--panel2)", padding: "1px 4px", borderRadius: 3 }}>tag:jwt</code>{" "}
            ...
            {tabs.map((t) => (
              <div
                ...
                style={{
                  ...
                  borderRight: "1px solid var(--border)",
                  borderBottom: t.relPath === activePath ? "1px solid var(--bg)" : "1px solid transparent",
                  marginBottom: -1,
                  cursor: "pointer",
                  whiteSpace: "nowrap",
                  borderTop: t.relPath === activePath ? "2px solid var(--accent)" : "2px solid transparent",
                }}
              >
                <span style={{ opacity: 0.6, fontSize: 11 }}>{t.relPath.includes("STATE") ? "◆" : "≡"}</span>
                {t.title}
                <span
            ...
            <div style={{ flex: 1, overflow: "auto", display: "flex", justifyContent: "center", padding: "16px 0" }}>
              <div style={{ width: 720, maxWidth: "92%" }}>
                ...
                <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginBottom: 12 }}>
                  ...
                  >
                    <b style={{ color: "var(--accent)", fontWeight: 500 }}>type</b> {note.type}
                  </span>
                  ...
                  {note.tags.length > 0 && (
                    ...
                    >
                      <b style={{ color: "var(--accent2)" }}>tags</b> {note.tags.join(" ")}
                    </span>
                  )}
                  {note.epic && (
                    <span
            ...
            <>
              ...
              <div style={{ flex: 1, overflow: "auto", padding: 12, display: "flex", flexDirection: "column", gap: 12 }}>
                ...
                <div style={{ fontSize: 11, color: "var(--muted)", lineHeight: 1.6 }}>
                  Use top bar: Depth 1/2, Full vault, and type/tag/feature filters. Click node → tab.
                </div>
                <div
                  style={{
                    background: "var(--bg)",
              ...
              <div style={{ flex: 1, overflow: "auto", padding: 12, display: "flex", flexDirection: "column", gap: 10 }}>
                ...
                {worklogData.entries.map((e) => (
                  ...
                  >
                    ...
                    />
                    <span style={{ fontSize: 12, color: "var(--text)", fontFamily: "Fragment Mono" }}>
                      {e.date}
                    </span>
      ...
      >
        ...
        <span style={{ opacity: 0.8 }}>index {workspaces.find((w) => w.id === activeWs)?.indexFresh ?? "…"}</span>
        <span style={{ opacity: 0.8 }}>vault: {workspaces.find((w) => w.id === activeWs)?.tildifiedKb ?? ""}</span>
        <button
