...
export function RightDock(props: Props): JSX.Element {
  ...
  if (isWorklogTimeline && worklogData) {
    return <WorklogJumpPanel worklog={worklogData} focus={worklogFocus} onJump={onJumpToDate} />;
  }
  return <NoteDock note={note} onOpen={onOpen} onWikilink={onWikilink} />;
}

function GraphInfoPanel({ activePath, graph }: { activePath: string; graph: GraphPayload | null }) {
