...
import { WrapGateHookResolver } from "@/modules/worklog/hooks/wrapGate/wrapGate.hook.ts";
...
test("registers the wrap-gate hook and handles a payload", async () => {
  const [handler] = registerHooks([WrapGateHookResolver], makeAppContext());
  if (handler === undefined) throw new Error("expected one hook handler");

  expect(handler.name).toBe(HookName.WrapGate);
