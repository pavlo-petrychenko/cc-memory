...
function makeDoctorContext(proc = makeProcFake()) {
  const ctx = makeAppContext({ proc });
  const container = ctx.gateways;
  // DoctorService reindexes the real index via `ctx.searchIndex`; a real
  // adapter keeps the "unreachable index" diagnostic exercisable.
  ctx.searchIndex = new SearchIndexAdapter(container.fs, (path) =>
    container.openDatabase(path),
  );
...
describe("DoctorService — per-workspace diagnostics", () => {
  test("a fully healthy workspace reports ok kb/worklogs and an ok, empty index", async () => {
    const { ctx, container } = makeDoctorContext();
    await container.fs.mkdir(workspaceFixture().kb);
    await container.fs.mkdir(workspaceFixture().worklogs);

    const report = await new DoctorService(ctx).gatherReport([workspaceFixture()], {
      repoRoot: REPO_ROOT,
      registryError: null,
    });
  ...
  test("reports the index as unreachable rather than throwing, when the db can't be opened", async () => {
    ...
    const report = await new DoctorService(ctx).gatherReport([brokenWorkspace], {
      repoRoot: REPO_ROOT,
      registryError: null,
    });
  ...
  test("reports wrap-state.json / inject.jsonl sizes from beside the index db", async () => {
    ...
    const report = await new DoctorService(ctx).gatherReport([workspace], {
      repoRoot: REPO_ROOT,
      registryError: null,
    });
...
describe("DoctorService — install/hooks/bun diagnostics", () => {
  test("hooks is null (and bun unreported) when there is no installed.json manifest", async () => {
    ...
    const report = await new DoctorService(ctx).gatherReport([], {
      repoRoot: REPO_ROOT,
      registryError: null,
    });
  ...
  test("reports the recorded bun path missing when the file no longer exists", async () => {
    const { ctx, container } = makeDoctorContext();
    await new ManifestService(ctx).save(
