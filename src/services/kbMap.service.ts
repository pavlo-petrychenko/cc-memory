...
import { parseIndexNote } from "../domain/note.ts";
import { tildify } from "../domain/paths.ts";
import type { KbMapFeature, KbMapInput } from "../domain/render/kbMap.renderer.ts";
 ...
 * `domain/render/kbMap.renderer.ts`, whose own doc comment names this file as
 ...
 * `services/index/build.ts`'s `joinUnderDir` rather than shared, per their own
 * precedent. */
