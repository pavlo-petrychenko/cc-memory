...
import { isUnder, sanitizeSlug } from "../domain/paths.ts";
import type { RawWorkspace, Workspace, WorktreeSlug } from "../domain/Workspace.ts";
...
/**
 * Resolve a `cwd` to exactly one workspace by longest-prefix match, or `null` if
 * `cwd` is under no workspace (`resolve.resolve`, `resolve.py:12-26`). This is the
 * encapsulation choke point (invariant #2): a session sees memory for the single
 * workspace returned here and nothing else.
 *
 * `cwd` arrives already expanded (`Env.cwd()` returns an `AbsPath` directly) —
 * unlike Python, which re-expands it itself (`resolve.py:16`), there is nothing
 * left to expand here.
 */
export function resolveWorkspace(
  ...
  for (const raw of raws) {
    ...
    for (const prefix of expanded.match) {
      ...
      // the prefix that actually won this cwd's resolution (`resolve.py:25`'s
      // `dict(ws, _prefix=prefix)`).
...
/**
 * A worktree's identity within a workspace (`resolve.slug`, `resolve.py:46-60`).
 * Prefers the git worktree root — so distinct git worktrees of one repo get
 * distinct slugs, and subdirs of a repo collapse to the repo root — but only when
 * that root actually lies inside the matched prefix; otherwise falls back to
 * `cwd` (which is guaranteed to be `ws.matchedPrefix` or nested under it, since
 * `ws` was itself resolved for this `cwd`).
 */
export async function worktreeSlug(
  ...
  if (toplevelOutput !== "") {
    ...
    // (unlike Python's redundant `registry.expand()` call at `resolve.py:36`).
  ...
  // `_sanitize` (`resolve.py:43`) is applied directly to the relative path: its
  // own per-character replacement already turns `/` into `-`, making Python's
  // preceding `rel.replace(os.sep, "-")` (`resolve.py:60`) a no-op worth skipping.
