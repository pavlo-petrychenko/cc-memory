 ...
 * `commands/hookDispatch/hookDispatch.command.ts`'s `hook()` is the one place
 * that supplies a REAL container for an actual invocation of `memory hook
 * <name>`.
 *
 * `HookContext`/`HookHandler` live here rather than in a `.typedefs.ts` file:
 * `HookContext` embeds the real `Container`, so a purely type-only file could
 * never define it without itself referencing `platform/` — the same
 * "impure by what it needs" reasoning that keeps this whole file a
 * `.service.ts`.
 */
