...
const DEFAULT_NEIGHBORS_LIMIT = 8;
 ...
 * one. Feeds the RRF corroboration bonus in `search.service.ts`'s
 * `searchFused`. A wikilink `dst` is resolved to a candidate by
 * relpath-minus-`.md` first (`relKey`), then by basename; self-links are
 * skipped. Returns an empty map for fewer than 2 candidates —
