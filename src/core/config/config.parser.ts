  ...
  parse(env: EnvSnapshot): Config {
    return {
      injectMinScore: this.parseNumber(
        env[ENV_INJECT_MIN_SCORE],
        INJECT_MIN_SCORE_DEFAULT,
      ),
      linkBoost: this.parseNumber(env[ENV_LINK_BOOST], LINK_BOOST_DEFAULT),
      injectLogEnabled: env[ENV_INJECT_LOG] !== INJECT_LOG_DISABLED_VALUE,
      blockAfter: this.parseNumber(env[ENV_BLOCK_AFTER], BLOCK_AFTER_DEFAULT),
      blockDrift: this.parseNumber(env[ENV_BLOCK_DRIFT], BLOCK_DRIFT_DEFAULT),
      gateDisabled: env[ENV_GATE_DISABLE] === GATE_DISABLED_VALUE,
      logLevel: this.parseLogLevel(env[ENV_LOG_LEVEL]),
    };
  }
}
